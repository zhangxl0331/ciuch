...
<div class="h_status">
	...
	查看日志<span class="pipe">|</span><a href="{{ url:base }}blog/uid-{{ space:uid }}-view-me.html">返回{{ helper:lang line="blog" }}列表</a>
</div>
...
<div{{if blog:related}} id="content"{{ endif }}>
	<div class="entry">
		<div class="title">
			...
			<a href="{{ url:base }}blog/uid-{{ space:uid }}-view-me-friend-{{ blog:friend }}.html" class="gray">
			...
			{{ if blog:tag }}
				...
				{{ /blog:tag }}
			{{ endif }}
			<span class="time">{{ date:sgmdate dateformat="Y-m-d H:i" time=blog:dateline format="1" }}</span>
		</div>
		...
		<div class="status">
			...
			<div class="r_option">
				{{ if member:uid == blog:uid }}
				<a href="cp.php?ac=blog&blogid=$blog[blogid]&op=edit">编辑</a><span class="pipe">|</span>
				<a href="{{ url:base }}blog/cp/delete?blogid={{ blog:blogid }}" id="blog_delete_{{ blog:blogid }}" onclick="ajaxmenu(event, this.id, 99999)">删除</a><span class="pipe">|</span>
				{{ endif }}
			...
			{{if blog:replynum }}<span class="pipe">|</span><span id="comment_replynum">{{ blog:replynum }}</span> 个评论{{ endif }}
		</div>
	</div>
	...
	<div class="trace" style="padding-bottom: 10px;">
		...
		<!--{if $tracelist}-->
			...
			<li>
				...
				<p><a href="{{ url:base }}space/uid-{{ space:uid }}.html" title="{$_SN[$value[uid]]}" target="_blank">{$_SN[$value[uid]]}</a></p>
			</li>
			<!--{/loop}-->
			<!--{if !$space[self]}-->
	...
	<div class="comments" id="div_main_content">
		...
		<div class="page">{{helper:page total_rows=blog:replynum per_page=30 per_page=input:page base_url=input:REQUEST_URI}}</div>
