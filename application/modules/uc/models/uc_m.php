...
class uc_m extends CI_Model
{
	...
	function get_user_by_uid($uid) {
		$arr = $this->db->where('uid', $uid)->get($this->_table)->row_array();
		return $arr;
	}

	function get_user_by_username($username) {
		$arr = $this->db->where('username', $username)->get($this->_table)->first_row('array');
		return $arr;
	}
	...
	function check_mergeuser($username) {
		$data = $this->db->select('count(*)')->where(array('appid'=>$this->input->get_post('appid'), 'username'=>$username))->get('mergemembers')->row_array();
		return $data;
	}
