...
// 		parent::__construct();		
		...
		$this->_ci->load->model('uc/user_m', 'uc_user_m');
	}
	 ...
	 * @return array (uid/status, username, password, email)
	 ...
	 -2 : �����
	 */
	function login($username, $password, $isuid = 0, $checkques = 0, $questionid = '', $answer = '') 
	{
		$isuid = intval($isuid);
		if($isuid) {
			$user = $this->_ci->uc_user_m->get_user_by_uid($username);
		} else {
			$user = $this->_ci->uc_user_m->get_user_by_username($username);
		}
	...
	public function avatar($uid, $size='middle', $type='')
	{
		...
		$avatar = 'data/avatar/'.$dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2).'_'.$type.'_avatar_'.$size.'.jpg';
		...
		{
			return "images/noavatar_$size.gif";
		}
	}
